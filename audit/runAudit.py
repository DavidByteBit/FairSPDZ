...
def run():
    ...
    if type_of_data.equals("model"):
        metadata = subprocess.check_output(
            [sys.executable,
             "/ProcessModelData/" + settings_map["model_type"] + ".py",
             settings_map])
...
def validate_settings(settings_map):
    ...
    if not path.exists(settings_map["path_to_private_data"]):
        error_msg = "Path to private data not found\n" \
                    "In your settings file, check \'path_to_private_data\' to confirm if it is correct\n"
